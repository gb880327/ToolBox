...
use crate::database::{new_wrapper, save_or_update, table_column, table_list, BaseModel};
...
impl Service {
    ...
    pub fn projects(&mut self) -> Option<Page<Project>> {
        ...
        let data: Page<Project> =
            block_on(super::RB.fetch_page_by_wrapper(new_wrapper(), &page_request)).unwrap();
        Some(data)
    }
    ...
    pub fn remove_project(&mut self) -> Option<bool> {
        ...
        Some(
            block_on(Project::remove(
                &super::RB,
                Some(new_wrapper().eq("id", project_id)),
            ))?
                > 0,
        )
    }
    ...
    pub fn servers(&mut self) -> Option<Page<Server>> {
        block_on(Server::list_by_page(
            &super::RB,
            None,
            self.param.as_ref()?.get_page_param()?,
        ))
    }
    ...
    pub fn deploy_setting(&mut self) -> Option<Vec<Command>> {
        let project_id = self.param.as_ref()?.get_int("id")?;
        block_on(Command::list(
            &super::RB,
            Some(new_wrapper().eq("project_id", project_id)),
        ))
    }
    ...
    pub fn save_deploy_setting(&mut self) -> Option<bool> {
        ...
        match block_on(super::RB.remove_by_column::<Command, _>("project_id", &project_id)) {
            Ok(_rst) => match block_on(super::RB.save_batch(&setting, &[])) {
    ...
    pub fn remove_datasource(&mut self) -> Option<bool> {
        ...
        Some(
            block_on(DataSource::remove(
                &super::RB,
                Some(new_wrapper().eq("id", ds_id)),
            ))?
                > 0,
        )
    }
    ...
    pub fn remove_category(&mut self) -> Option<bool> {
        let id = self.param.as_ref()?.get_int("id")?;
        Some(
            block_on(Category::remove(
                &super::RB,
                Some(new_wrapper().eq("id", id)),
            ))?
                > 0,
        )
    }
    ...
    pub fn gen_setting(&mut self) -> Option<GenProject> {
        ...
        let gen_project: Option<GenProject> = block_on(GenProject::one(
            &super::RB,
            new_wrapper().eq("project_id", project_id),
        ));
        gen_project
    }
    ...
    pub fn table_and_template(&mut self) -> Option<GenData> {
        let project_id = self.param.as_ref()?.get_int("projectId")?;
        let gen_project: GenProject = block_on(GenProject::one(
            &super::RB,
            new_wrapper().eq("project_id", project_id),
        ))?;
        ...
        block_on(super::MYSQL.link(&format!(
            "mysql://{}:{}@{}:{}/{}",
            db_info.user?,
            db_info.password?,
            db_info.host?,
    ...
    pub fn gen_template(&mut self) -> Option<bool> {
        ...
        let gen_project: GenProject = block_on(GenProject::one(
            &super::RB,
            new_wrapper().eq("project_id", gen_info.project_id),
        ))?;
        let project: Project = block_on(Project::one(
            &super::RB,
            new_wrapper().eq("id", gen_info.project_id),
        ))?;
        ...
        for table in tables.iter_mut() {
            ...
            let column: Result<Vec<Column>, Error> =
                block_on(table_column(&db_name, &table.org_name.as_ref()?));
            match column {
                Ok(mut cols) => {
        ...
        for tpl in templates.iter() {
            let tp: Template = block_on(Template::one(
                &super::RB,
                new_wrapper().eq("id", tpl.template_id),
            ))?;
